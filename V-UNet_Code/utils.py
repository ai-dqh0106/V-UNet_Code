...
class FocDiceLoss(nn.Module):
    def __init__(self, wb=0.4, wd=0.6):
        super(FocDiceLoss, self).__init__()
        self.focloss = FocalLoss()
        self.diceloss = DiceLoss()
        self.wb = wb
        self.wd = wd

    def forward(self, pred, target):
        foc = self.focloss(pred, target)
        dice = self.diceloss(pred, target)
        # diceloss = self.dice(pred, target, softmax=True)

        loss = self.wb * foc + self.wd * dice
        return loss

class FocBceLoss(nn.Module):
    def __init__(self, wb=0.4, wd=0.6):
        super(FocBceLoss, self).__init__()
        self.focloss = FocalLoss()
    ...
    def forward(self, pred, target):
        foc = self.focloss(pred, target)
        bce = self.bceloss(pred, target)
        # diceloss = self.dice(pred, target, softmax=True)

        loss = self.wb * foc + self.wd * bce
        return loss


class CEDiceLoss(nn.Module):
    def __init__(self, wb=0.6, wd=1):

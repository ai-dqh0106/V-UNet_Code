...
class Block(nn.Module):
    ...
    def __init__(self, dim, drop_path=0., layer_scale_init_value=1e-6, H_SS2D=H_SS2D, d_state=16, d_conv=4, expand=2):
        super().__init__()
        ...
        self.mamba = Mamba(
            d_model=dim // 4,  # Model dimension d_model
    ...
    def forward(self, x):
        ...
        x = self.msc(x)
        x0 = self.norm1(x)
        x_norm = self.norm0(x_flat)

        x1, x2, x3, x4 = torch.split(x_norm, 4, dim=2)
        ...
        M, N = x_mamba.shape[:2]
        x_mamba = x_mamba.reshape(M, N, *img_dims)
        x = x + self.drop_path(gamma1 * self.H_SS2D(x0))
        x = x + x_mamba
...
class V_UNet(nn.Module):
    def __init__(self, num_classes=1, input_channels=3, layer_scale_init_value=1e-6, H_SS2D=H_SS2D, block=Block,
                 pretrained=None,
        ...
        if not isinstance(H_SS2D, list):
            H_SS2D = [partial(H_SS2D, order=2, s=1 / 3, gflayer=Filter_SS2D),
                      partial(H_SS2D, order=3, s=1 / 3, gflayer=Filter_SS2D),
                      partial(H_SS2D, order=4, s=1 / 3, h=24, w=13, gflayer=Filter_SS2D),
                      partial(H_SS2D, order=5, s=1 / 3, h=12, w=7, gflayer=Filter_SS2D)]
        else:
            H_SS2D = H_SS2D
            assert len(H_SS2D) == 4
